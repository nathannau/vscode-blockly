...
- **"static" checkbox on the two function-definition blocks** — same idea as "declare variable"/"declare array": check it for a helper that's purely internal to its file (avoids a link error if another file happens to define a same-named one), leave it unchecked for a function that's part of this file's public interface (e.g. `jeu1_start`/`jeu1_loop`/`jeu1_stop` called from `main.cpp`).
...
- **"get variable"/"set variable"'s picker no longer lists Constants/Arrays either** — same restriction, now also applied to Blockly's own built-in `variables_get`/`variables_set` (and `math_change`, not currently exposed in any toolbox category but patched for consistency) via their field's (technically private, but stable) `setTypes` method — these aren't custom block defs, so they couldn't take a `variableTypes` argument the way `declare_variable` did.
