...
- **"Declare array" block** (Arrays category) — declares a new fixed-size, zero-initialized array (element type + size); `array get`/`array set` only ever index into one that already exists. Has its own explicit `static` checkbox, on top of the automatic static-in-a-secondary-file behavior it already had.
